...
//
// It is the brittle part of this job — it parses two HTML forms — which is why
// syncIdentity reuses an already-stored token whenever that token still works,
// and why hiddenInputs is unit tested rather than only exercised in anger.
...
func (a *forgeAuth) createToken(ctx context.Context) (string, error) {
	body, err := a.do(ctx, http.MethodPost, a.woodpeckerURL+"/api/user/token", nil, "")
	if err != nil {
