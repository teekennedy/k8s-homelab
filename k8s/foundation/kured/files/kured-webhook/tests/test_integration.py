...
def start_http_server(handler_cls: type[BaseHTTPRequestHandler]) -> tuple[HTTPServer, threading.Thread]:
...
def wait_for_requests(recorder: RequestRecorder, count: int, timeout: float = 5.0) -> None:
    ...
    with recorder.lock:
        raise AssertionError(
            f"expected {count} requests, got {len(recorder.requests)}"
        )
...
def test_webhook_uncordon_sends_silences_and_resolves_alert(alertmanager_server: tuple) -> None:
    recorder, url, _, _ = alertmanager_server
    previous = server.CONFIG
    server.CONFIG = build_webhook_config(url)
    webhook, webhook_thread = start_http_server(server.Handler)
